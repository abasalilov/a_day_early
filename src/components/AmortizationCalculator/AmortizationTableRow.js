...
const isLastPayment = props => {
  ...
  return (
    props.row.paymentNumber ===
    amortization[amortization.length - 1].paymentNumber
  );
};
...
const AmortizationTableRowComponent = props => (
  <Container>
    <Row>
    ...
    {shouldShowAnnualSummaryRow(props) && (
      <Row style={summaryRowStyle}>
        ...
        {displaySummaryCell(
          displayAnnualTotal(props, "principalPaymentRounded")
        )}
        {displaySummaryCell(
          displayAnnualTotal(props, "interestPaymentRounded")
        )}
        {displaySummaryCell(cf(props.row.accInterestRounded, { code: "USD" }))}
        {displaySummaryCell(
          cf(props.row.principalBalanceRounded, { code: "USD" })
        )}
      </Row>
    )}
  </Container>
...
export const AmortizationTableRow = AmortizationTableRowComponent;
